...
rules_x86 = {
    ...
    '$code9': '{C7 4? ?? ?? ?? 03 00}',
    # get size strict 2
    '$code10': '{C7 8? ?? ?? ?? FF ?? ?? 03 00}',

}

rules_x64 = {
    '$code1': '{48 C7 (84| 44) 24 (?? ?? ?? ??| ??) ?? 4? 03 00 (C7 84| C7 44| E8)}',
    '$code2': '{8? [4] 00 00 C1 ?? 07 89}',
    '$code3': '{48 C7 (84| 44) 24 (?? ?? ?? ??| ??) ?? ?? ?? 00 (C7 84 24| C7 44 24| E8)}',
    '$code4': '{FF 15 [2] 00 00 [4-40] 48 8D 05 [4-40] 48 C7 (84| 44) 24 (?? ?? ?? ??| ??) ?? ?? ?? 00}'
}
        ...
        for rule in ['$code9', '$code10']:
            ...
            for match in matches:
                try:
                    ...
                    exec_size = struct.unpack('I', opcodes[3 + iddx: 3 + 4 + iddx])[0]
